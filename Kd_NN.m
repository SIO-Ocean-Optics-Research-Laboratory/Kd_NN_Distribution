...
%RICK; IN LS2 THE ORDER HAD sza AND lambda BEFORE Rrs, SHOULDN'T WE FOLLOW SAME LOGIC?
...
%   the output wavelength lambda can be either the same or can differ between
%   the "sample" inputs which depends on how lambda(nsampx1) is defined by
...
%
%Outputs: Kd_est
%   Kd_est (mx1 Double): The estimated value of the average diffuse %RICK; WHY mx1? SHOULDN'T IT BE nsamplx1?
...
%% Check function arguments and existence of LUTs
    ...
    %copy lambda values into an array to match the number of samples if
    %only one wavelength lambda is provided
    if length(lambda)==1
    ...
    %RICK; WE SHOULD DESCRIBE WHAT muw IS SIMILAR TO IN LS2
    ...
    %RICK; I'M NOT SURE WHY WE NEED TO CALCULATE BOTH CASE 1 and CASE 2 FOR A GIVEN INPUT?
    %SHOULDN'T WE FIRST DO A CHECK OF THE Rrs RATIO, DECIDE WHICH MODEL IS APPLICABLE, AND THEN USE A SWITCH TO BUILD THE APPROPRIATE NEURAL NET?
    %THIS WOULD BE MORE EFFICIENT AND LOGICAL THAN JUST ALWAYS CALCULATING BOTH
    %ALSO, WHAT IS THE LOGIC OF DOING CASE 2 FIRST FOLLOWED BY CASE 1?
    %I'M ALSO UNSURE IF THE CASE 1/CASE 2 DESCRIPTION WAS USED IN CEDRIC'S PAPER, I THOUGHT HE CALLED IT TURBIDITY. SHOULD WE BE CONSISTENT WITH
    %HIS PAPER, EVEN IF IT'S NOT 100% ACCURATE TERMINOLOGY?
    ...
    %read case 2 NN LUT as weight and bias matrices 
    ...
    Kd_est = Kd_est';
    %RICK; SO I AM CONFUSED HERE BECAUSE THE OUTPUT VARIABLE APPEARS TO HAVE 2 SOLUTIONS (CASE 1 AND CASE 2), NOT 1
    %ARE YOU OUTPUTTING BOTH? ONLY THE RELEVANT ONE SHOULD BE ASSIGNED TO OUTPUT
