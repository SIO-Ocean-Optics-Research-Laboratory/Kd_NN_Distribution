function [Kd]=Kd_NN_PACE(Rrs,sza,lambda,Kd_NN_LUT_PACE)
...
%
%   Kd_NN_LUT_PACE [1x1 Structure]: Structure containing three required
%   look-up tables (LUTs); can be loaded via load('Kd_NN_LUT_MODIS.mat')
%
%       Kd_NN_LUT_PACE.weights_1: LUT with weights and biases from NN for
%       clear waters (where Rrs(488)/Rrs(547) >= 0.85)
%
%       Kd_NN_LUT_PACE.weights_2: LUT with weights and biases from NN for
%       turbid waters (where Rrs(488)/Rrs(547) < 0.85)
%
%       Kd_NN_LUT_PACE.train_switch: LUT with means and standard
...
%2020-03-23: Original MATLAB version, D. Jorge Schaffer 2022-09-01:
%Revised MATLAB version, M. Kehrli 2022-11-03: Final revised MATLAB version
%M. Kehrli, R. A. Reynolds and D. Stramski (v1.0)
%2023-10-10: Corrected weights and biases in KdNN LUT for clear waters
%(v1.1) 
%2024-03-08: New version of the KdNN developed for PACE inputs provided by 
%Daniel Schaffer
%(v2.1) 2024-09-05: Updated version of PACE NN using a new training dataset
%that led to a new NN architecture with new weights and biases of the KdNN
%(v2.2) 2024-09-25: Updated version of PACE NN using a new training dataset
%that led to a new NN architecture with new weights and biases of the KdNN
%(v2.3)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Check function arguments and existence of LUTs
    ...
    train_switch = Kd_NN_LUT_PACE.train_switch;
    ...
    %type of the input Rrs spectrum (490/560)
    ratio = inputs(3)/inputs(6);
    
    %build NN for clear waters
    if ratio >= 0.85

        %read in NN weights and biases for clear waters
        weights_1 = Kd_NN_LUT_PACE.weights_1;
        ...
        %training dataset; remove Rrs(670) and Rrs(700) data for clear waters
        mu = mu_switch([1:10,13:15]);
        std = std_switch([1:10,13:15]);
    ...
    elseif ratio < 0.85
        ...
        weights_2 = Kd_NN_LUT_PACE.weights_2;
